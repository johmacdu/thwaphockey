...
// --- Team weekly plan -------------------------------------------------------
// Which of the 3 categories are ON each weekday. Shape:
//   { mon:['stick','shoot','dryland'], tue:[...], ..., sun:[...] }
// Default (no plan set) is all three every day, matching the built-in rotation.
const PLAN_DAYS = ['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'];
const PLAN_CATS = ['stick', 'shoot', 'dryland'];
function defaultPlan() {
  const p = {};
  for (const d of PLAN_DAYS) p[d] = PLAN_CATS.slice();
  return p;
}
export async function getTeamPlan(code) {
  const raw = await kv.get(teamPlanKey(code));
  if (!raw || typeof raw !== 'object') return defaultPlan();
  // Normalize: keep only known days/cats, fall back to default for a missing day.
  const out = {};
  for (const d of PLAN_DAYS) {
    const cats = Array.isArray(raw[d]) ? raw[d].filter((c) => PLAN_CATS.includes(c)) : null;
    out[d] = cats || PLAN_CATS.slice();
  }
  return out;
}
export async function setTeamPlan(code, plan) {
  ...
  for (const d of PLAN_DAYS) {
    const cats = Array.isArray(src[d]) ? src[d].filter((c) => PLAN_CATS.includes(c)) : [];
    out[d] = cats;
  }
