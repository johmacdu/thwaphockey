...
const teamGoalKey = (code) => `teamGoal:${codeNorm(code)}`;
...
export async function setTeam(code, obj) {
  ...
  // Maintain a small index of all team codes so admin views can scan every team.
  const raw = await kv.get(teamsIndexKey());
  const idx = Array.isArray(raw) ? raw : [];
  if (!idx.includes(rec.code)) { idx.push(rec.code); await kv.set(teamsIndexKey(), idx); }
  return rec;
}

// All known team codes (from the index setTeam maintains).
export async function listAllTeamCodes() {
  const raw = await kv.get(teamsIndexKey());
  return Array.isArray(raw) ? raw : [];
}

// Backfill: ensure a team code is present in teams:index without rewriting the
// team record. Teams created before the index existed are missing from it, so
// admin views that scan the index would not see them. Idempotent.
export async function ensureTeamIndexed(code) {
  const c = codeNorm(code);
  const raw = await kv.get(teamsIndexKey());
  const idx = Array.isArray(raw) ? raw : [];
  if (!idx.includes(c)) { idx.push(c); await kv.set(teamsIndexKey(), idx); }
  return idx;
}
...
export async function addMember(code, { playerId, firstName, lastName, number, position, parentEmail, parentEmail2, status, photo }) {
  ...
  const rec = {
    ...
    parentEmail2: emailNorm(parentEmail2),
...
// Update an existing member's editable fields (name, number, position, photo).
// Does not move the member between teams or change identity keys. Returns the
// updated record, or null if the member does not exist.
export async function updateMember(playerId, { firstName, lastName, number, position, gamePosition, photo, parentEmail, parentEmail2 } = {}) {
  const pid = idNorm(playerId);
  const rec = await kv.get(memberKey(pid));
  if (!rec) return null;
  const next = { ...rec };
  if (typeof firstName === 'string' && firstName.trim()) next.firstName = firstName.trim();
  if (typeof lastName === 'string') next.lastName = lastName.trim();
  if (number !== undefined) next.number = (number == null || number === '') ? null : Number(number);
  if (typeof position === 'string') next.position = position;
  // gamePosition is the PLAYER's own game-day choice ('F'|'D'|'B'), separate from
  // the coach-owned roster `position`. Only these three values are accepted.
  if (typeof gamePosition === 'string' && ['F', 'D', 'B'].includes(gamePosition)) next.gamePosition = gamePosition;
  if (typeof photo === 'string' && photo.startsWith('data:image/')) next.photo = photo.slice(0, 400000);
  if (typeof parentEmail === 'string') next.parentEmail = emailNorm(parentEmail);
  if (typeof parentEmail2 === 'string') next.parentEmail2 = emailNorm(parentEmail2);
  await kv.set(memberKey(pid), next);
  return next;
}
...
// --- Admin ------------------------------------------------------------------
// An email-keyed admin account (separate namespace from coaches). Stores only a
// salted password hash (never plaintext). passHash is null until first-login set.
const adminKey = (email) => `admin:${emailNorm(email)}`;

export async function getAdmin(email) {
  return (await kv.get(adminKey(email))) || null;
}

export async function setAdmin(email, obj) {
  const rec = { ...obj, email: emailNorm(email) };
  await kv.set(adminKey(email), rec);
  return rec;
}

// --- Coach password reset tokens (single-use, short-lived) ------------------
const resetKey = (token) => `coachReset:${String(token).replace(/[^a-zA-Z0-9]/g, '')}`;
const RESET_TTL_MS = 30 * 60 * 1000; // 30 minutes
export async function createResetToken(email) {
  const token = randToken();
  await kv.set(resetKey(token), { email: emailNorm(email), exp: Date.now() + RESET_TTL_MS });
  return token;
}
// Consume a reset token: returns the email if valid+unexpired, else null. Single-use.
export async function consumeResetToken(token) {
  const k = resetKey(token);
...
function randToken() {
  // 24 hex chars, no crypto import needed here (teams_store stays dep-light).
  let s = '';
...
export async function addAssistantCoach(code, email, opts = {}) {
  ...
  const entry = { email: e, role: 'assistant', addedAt: new Date().toISOString() };
  if (typeof opts.name === 'string' && opts.name.trim()) entry.name = opts.name.trim();
  if (typeof opts.photo === 'string' && opts.photo.startsWith('data:image/')) entry.photo = opts.photo.slice(0, 400000);
...
// --- Team goal (one shared game-day focus the whole team trains) ------------
// Coach sets ONE goal for the team; it fills a slot in every player's Game Day
// plan. Stored per team, month-stamped like the IDP goal. Empty text clears it.
export async function getTeamGoal(code) {
  return (await kv.get(teamGoalKey(code))) || null;
}

export async function setTeamGoal(code, { text }) {
  const clean = String(text || '').trim();
  if (!clean) { await kv.set(teamGoalKey(code), null); return null; }
  const rec = {
    text: clean,
    month: localMonthStr(),
    setBy: 'coach',
  ...
  await kv.set(teamGoalKey(code), rec);
  return rec;
}
...
// Thwap review: flip a suggestion's status (new -> accepted | dismissed).
export async function setSuggestionStatus(code, id, status) {
  const ok = ['new', 'accepted', 'dismissed'];
  if (!ok.includes(status)) return null;
  const raw = await kv.get(drillSuggestKey(code));
  const list = Array.isArray(raw) ? raw : [];
  let hit = null;
  for (const s of list) { if (s && s.id === id) { s.status = status; hit = s; } }
  if (!hit) return null;
  await kv.set(drillSuggestKey(code), list);
  return hit;
}
