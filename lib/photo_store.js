...
export async function recordConsent(kv, playerId, email, now = Date.now()) {
  await kv.set(consentKey(playerId), { method: 'email', emailHash: sha256(String(email).toLowerCase()), grantedAt: now });
}

// Admin-recorded consent for the case where a parent gave permission out of band
// (text, in person, written) rather than through the email-code flow. This does
// NOT pretend to be a verified email: it records HOW consent was given, WHO gave
// it, and WHICH admin recorded it, so the stored record is truthful and auditable.
const CONSENT_METHODS = ['email', 'text', 'in_person', 'written'];
export async function recordAdminConsent(kv, playerId, { method, grantedBy, recordedBy } = {}, now = Date.now()) {
  const m = CONSENT_METHODS.includes(method) ? method : 'other';
  await kv.set(consentKey(playerId), {
    method: m,
...
export async function getConsent(kv, playerId) {
  const rec = await kv.get(consentKey(playerId));
