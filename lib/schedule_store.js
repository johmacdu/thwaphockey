...
// Set (or clear) the coach's team-focus goal on one existing event. focus is
// { goalId, goalTitle } or null to clear. Returns the updated schedule, or null
// if the event id is not found.
export async function setEventFocus(code, eventId, focus) {
  const s = await getSchedule(code);
  const ev = (s.events || []).find((e) => e.id === eventId);
  if (!ev) return null;
  if (focus && focus.goalId) {
    ev.focus = { goalId: String(focus.goalId), goalTitle: String(focus.goalTitle || '') };
  } else {
    delete ev.focus;
  }
  await setSchedule(code, s);
  return s;
}
