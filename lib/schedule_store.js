...
export async function addEvent(code, event) {
  ...
  // A caller-supplied id is used as-is (so re-adding the same id updates in place).
  // An auto-generated id gets a random suffix so two id-less adds in the same
  // millisecond do not collide (which would drop the first).
  const id = event.id || ('g-' + Date.now() + '-' + Math.random().toString(36).slice(2, 7));
